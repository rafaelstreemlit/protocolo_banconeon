...
def get_db_connection():
    db_credentials = st.secrets["db_credentials"]  # Acessa o bloco "db_credentials" no secrets.toml
    ...
    return psycopg2.connect(
        ...
        port=db_credentials.get("port", "5432")  # Usa a porta padrão 5432 caso não esteja especificada
    )

def create_excel_with_model(data):
    # Caminho relativo para o modelo de Excel
    file_path = 'modelo_devolucao.xlsx'

    # Verificar se o arquivo existe
    if not os.path.exists(file_path):
        raise FileNotFoundError(f"Arquivo não encontrado no caminho especificado: {file_path}")
    ...
    with tab4:
        ...
        with st.expander("Excluir Todos os Registros"):
            ...
            if st.button("Excluir Todos os Registros", key="btn_delete_all"):
                admin_credentials = st.secrets["admin"]  # Acessa o bloco "admin" no secrets.toml
                
                if password_input == admin_credentials["delete_password"]:
                    if delete_all_records(password_input):
                        st.success("Todos os registros foram excluídos com sucesso.")
                    else:
                        st.error("Ocorreu um erro ao tentar excluir os registros.")
                else:
